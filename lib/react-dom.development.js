 ...
 * The rest of ReactDOM code should  top level types from this file.
 */
...
function finishEventHandler() {
  // Here we wait until all updates have propagated, which is ant
...
// TODO: direct s like some-package/src/* are bad. Fix me.
...
// Intentionally not named s because Rollup would use dynamic dispatch for
// CommonJS interop named s.
...
function handleTopLevel(bookKeeping) {
  ...
  // It's ant that we build the array of ancestors before calling any
...
/**
 * Some ant event plugins included by default (without having to require
 * them).
 */
...
// if a component just s ReactDOM (e.g. for findDOMNode).
...
function hideInstance(instance) {
  ...
  if (typeof style.setProperty === 'function') {
    style.setProperty('display', 'none', 'ant');
  } else {
    style.display = 'none';
  }
}
...
function invalidateContextProvider(workInProgress, type, didChange) {
  if (disableLegacyContext) {
    ...
    if (didChange) {
      ...
      // It is ant to unwind the context in the reverse order.
          ...
          if (lazyComponent._status === Pending) {
            var defaultExport = moduleObject.default;
            {
              if (defaultExport === undefined) {
                warning$1(false, 'lazy: Expected the result of a dynamic () call. ' + 'Instead received: %s\n\nYour code should look like: \n  ' + "const MyComponent = lazy(() => ('./MyComponent'))", moduleObject);
              }
            }
...
//
// The priority is still ant, though. When processing the update queue
    ...
    if ('contextType' in ctor) {
      ...
      if (!isValid && !didWarnAboutInvalidateContextType.has(ctor)) {
        ...
        if (contextType === undefined) {
          addendum = ' However, it is set to undefined. ' + 'This can be caused by a typo or by mixing up named and default s. ' + 'This can also happen due to a circular dependency, so ' + 'try moving the createContext() call to a separate file.';
...
// The scheduler is ed here *only* to detect whether it's been mocked
// DEV stuff
var ceil = Math.ceil;
      ...
      default:
        {
          ...
          var info = '';
          {
            if (type === undefined || typeof type === 'object' && type !== null && Object.keys(type).length === 0) {
              info += ' You likely forgot to export your component from the file ' + "it's defined in, or you might have mixed up default and " + 'named s.';
            }
...
// TODO: this is special because it gets ed during build.
